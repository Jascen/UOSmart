...
{
	public class HealingFairy : BaseHealer
	{
		...
		public HealingFairy( Serial serial ) : base( serial )
		{
		}
	...
	public class SummonedHealingFairy : HealingFairy
	{
		...
		[Constructable]
		public SummonedHealingFairy() : base()
		{
			m_HealTime = DateTime.Now + TimeSpan.FromSeconds( 3 );
		}

		public SummonedHealingFairy( Serial serial ) : base( serial )
		{
		}
		
		public override void OnMovement( Mobile m, Point3D oldLocation )
		{
			if ( InRange( m, 4 ) &&  ( m_HealTime <= DateTime.Now ) )
			{
				CheckForNeeds ( m );
				m_HealTime = DateTime.Now + TimeSpan.FromSeconds( 3 );
			}
